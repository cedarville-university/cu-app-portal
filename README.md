...
For apps developed locally with Codex that are not on GitHub yet, the portal can create an empty managed repository in the shared org first. The onboarding wizard then provides a Codex-ready prompt to inspect the local project, preserve any existing history, keep secrets out of the upload, add the managed repository as a remote, and push the code without requiring GitHub CLI. The user confirms **My code has been uploaded** only after Codex reports success; the portal then inspects and prepares that uploaded code.

Every Codex handoff is local-project-first. The wizard tells Windows users to install Git from **Company Portal** and macOS users to install it from **CedarNet 2.0**, then completely reopen Codex. A generated app uses a new empty folder; a local app uses its existing folder. The user creates a **local Codex project**, makes that app folder primary, and starts the task inside the project instead of using Quick chat or a standalone task. The generated prompts verify both the project folder and `git --version`, never attempt software installation, and use HTTPS with secure browser or operating-system GitHub sign-in instead of passwords, tokens, SSH keys, GitHub CLI, or the GitHub plugin.
